...
SEED = 666
JAX_TRAIN_RATIO = 0.71
JAX_VAL_RATIO = 0.06
JAX_TEST_RATIO = 0.23
OMA_TRAIN_RATIO = 0.00
OMA_VAL_RATIO = 0.00
OMA_TEST_RATIO = 1.00
...
def main():
    ...
    print(f"Seed: {SEED}")
    print(f"JAX tile train ratio: {JAX_TRAIN_RATIO}")
    print(f"JAX tile val ratio: {JAX_VAL_RATIO}")
    print(f"JAX tile test ratio: {JAX_TEST_RATIO}")
    print(f"OMA tile train ratio: {OMA_TRAIN_RATIO}")
    print(f"OMA tile val ratio: {OMA_VAL_RATIO}")
    print(f"OMA tile test ratio: {OMA_TEST_RATIO}")
    ...
    jax_train_data_list, jax_val_data_list, jax_test_data_list = random_split(jax_data_list, [JAX_TRAIN_RATIO, JAX_VAL_RATIO, JAX_TEST_RATIO])
    oma_train_data_list, oma_val_data_list, oma_test_data_list = random_split(oma_data_list, [OMA_TRAIN_RATIO, OMA_VAL_RATIO, OMA_TEST_RATIO])
    ...
    print("Done")
    print("--------------------------------")
    print(f"JAX tile train split: {JAX_TRAIN_RATIO}, {len(jax_train_data_list)} images")
    print(f"JAX tile val split: {JAX_VAL_RATIO}, {len(jax_val_data_list)} images")
    print(f"JAX tile test split: {JAX_TEST_RATIO}, {len(jax_test_data_list)} images")
    print(f"OMA tile train split: {OMA_TRAIN_RATIO}, {len(oma_train_data_list)} images")
    print(f"OMA tile val split: {OMA_VAL_RATIO}, {len(oma_val_data_list)} images")
    print(f"OMA tile test split: {OMA_TEST_RATIO}, {len(oma_test_data_list)} images")
    print("--------------------------------")
    ...
    with open(os.path.join(DEST_ROOT, SPLIT_INFO_BASENAME), "w") as f:
        f.write(f"JAX tile train ratio: {JAX_TRAIN_RATIO}\n")
        f.write(f"JAX tile val ratio: {JAX_VAL_RATIO}\n")
        f.write(f"JAX tile test ratio: {JAX_TEST_RATIO}\n")
        f.write(f"OMA tile train ratio: {OMA_TRAIN_RATIO}\n")
        f.write(f"OMA tile val ratio: {OMA_VAL_RATIO}\n")
        f.write(f"OMA tile test ratio: {OMA_TEST_RATIO}\n")
